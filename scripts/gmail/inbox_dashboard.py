"""
Inbox Archeology — Exploratory Dashboard (Streamlit)

Refactor notes:
- Kept original behavior (reads CSVs from ../output relative to this file).
- Added an optional CLI flag via Streamlit query params or env var:
  - INBOX_ARCH_OUTPUT_DIR: override where to read CSV outputs from.

Why: for packaging, you'll want to point the dashboard at a per-user work directory.
"""
...
DEFAULT_OUTPUT_DIR = os.path.abspath(os.path.join(THIS_DIR, "..", "output"))

# Allow override via env var (easy for launch scripts / suite runner)
OUTPUT_DIR = os.environ.get("INBOX_ARCH_OUTPUT_DIR", DEFAULT_OUTPUT_DIR)

REL_CLEAN = os.path.join(OUTPUT_DIR, "relationships_clean.csv")
CORE_TL = os.path.join(OUTPUT_DIR, "core_timeline.csv")
