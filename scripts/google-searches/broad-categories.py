...
CATEGORY_MAPPING = {
    "People": ["births", "deaths", "actors", "scientists", "politicians", "writers", "athletes", "musicians", "philosophers", "artists"],
    "Movies": ["films", "movies", "tv series", "television", "animated", "horror", "comedy", "drama"],
    "Places": ["countries", "cities", "towns", "villages", "regions", "states of", "provinces", "continents", "landmarks"],
    "Things": ["science", "technology", "concepts", "history", "theories", "inventions", "objects", "ideas"]
}
...
def get_wikipedia_categories(page_title):
    ...
    for page in pages.values():
        if "categories" in page:
            return [cat["title"].replace("Category:", "").lower() for cat in page["categories"]]
...
def categorize_wikipedia_pages(urls):
    """Broadly categorize Wikipedia pages."""
    broad_categories = defaultdict(list)
    
    for url in urls:
        title = extract_page_title(url)
        if title:
            categories = get_wikipedia_categories(title)
            matched_category = "Other"  # Default category if no match
            
            for broad_cat, keywords in CATEGORY_MAPPING.items():
                if any(keyword in cat for cat in categories for keyword in keywords):
                    matched_category = broad_cat
                    break
            
            broad_categories[matched_category].append(url)
            time.sleep(1)  # Prevent API rate limits
    
    return broad_categories

def save_to_txt(category_groups, filename="broad_categorized_urls.txt"):
...
def save_to_csv(category_groups, filename="broad_categorized_urls.csv"):
    """Save results to a CSV file."""
    with open(filename, "w", newline="", encoding="utf-8") as file:
        writer = csv.writer(file)
        writer.writerow(["Broad Category", "Wikipedia URL"])
        
        for category, pages in category_groups.items():
            for page in pages:
                writer.writerow([category, page])

def save_to_json(category_groups, filename="broad_categorized_urls.json"):
