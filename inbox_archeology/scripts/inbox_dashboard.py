...
def main(go_home: Optional[Callable[[], None]] = None) -> None:
    # -----------------------------
    ...
    # -----------------------------
    # App config (only if not embedded in the suite router)
    # -----------------------------
    if not st.session_state.get("_dd_suite", False):
    ...
    SUITE_OVERRIDE = (st.session_state.get("dd_inbox_arch_outdir") or "").strip()
    ...
    # -----------------------------
    # Header + Back button (when embedded)
    # -----------------------------
    top_left, top_right = st.columns([0.8, 0.2])
    with top_left:
        st.title("Inbox Archeology — Exploratory Dashboard")
        st.caption("Explore relationship volume, reciprocity, and lifecycle — all local-first.")
    with top_right:
    ...
    if OUTPUT_OVERRIDE:
        ...
        OUT_DIR = Path(SUITE_OVERRIDE).expanduser().resolve()
        st.sidebar.caption(f"Reading outputs from latest run:\n{OUT_DIR}")
    else:
    ...
    with right:
        ...
        # Plotly log axes cannot display zeros. Use +1 transformed columns so
        # relationships with 0 sent/received still render (and remain comparable).
        scat["received_plot"] = scat["received_by_me"].clip(lower=0) + 1
        scat["sent_plot"] = scat["sent_by_me"].clip(lower=0) + 1
        ...
        fig_rec.update_xaxes(title="Received by me (+1, log)")
        fig_rec.update_yaxes(title="Sent by me (+1, log)")
