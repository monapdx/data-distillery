"""Print summary stats from relationships_filtered.csv.

Refactor notes:
- Removed hardcoded paths.
- Added CLI args (--in, --top).
"""
...
def analyze_relationships(in_path: str, top_n: int = 30) -> None:
    ...
    with in_path.open("r", encoding="utf-8", newline="") as f:
        ...
        for r in reader:
            ...
            reciprocity = None if recv == 0 else sent / recv
...
def main():
    p = argparse.ArgumentParser(description="Analyze relationships_filtered.csv.")
    p.add_argument("--in", dest="inp", default=str(Path("output")/"relationships_filtered.csv"))
    p.add_argument("--top", type=int, default=30)
    args = p.parse_args()
    analyze_relationships(args.inp, args.top)
